...
const OnlineToggle = ({ isOnline, onToggle, loading }) => (
  <button
    ...
    className={`relative flex items-center gap-3 px-4 py-2 rounded-full border font-medium text-sm
      ...
      ${isOnline ? "bg-emerald-900/40 border-emerald-600/50 text-emerald-400" : "bg-slate-800/60 border-slate-600/50 text-slate-400"}
      disabled:opacity-50`}
  >
    <span className={`w-2.5 h-2.5 rounded-full transition-colors ${isOnline ? "bg-emerald-400 animate-pulse" : "bg-slate-500"}`} />
...
const BookingRequestModal = ({ request, onAccept, onReject }) => {
  ...
  return (
    <div style={{ position: "absolute", inset: 0, zIndex: 50, background: "rgba(0,0,0,0.65)", display: "flex", alignItems: "flex-end", padding: "1rem" }}>
      <div className="w-full max-w-lg mx-auto bg-slate-900 border border-slate-700 rounded-2xl shadow-2xl overflow-hidden animate-slide-up">
        <div className="h-1.5 bg-slate-700">
          <div className="h-full bg-[#0077B6] transition-all duration-1000" style={{ width: `${pct}%` }} />
        </div>
        <div className="p-5 space-y-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <span className="text-slate-100 font-semibold text-base">New Booking Request</span>
              <span className={`text-xs px-2 py-0.5 rounded-full border font-semibold ${typeStyle.color}`}>{typeStyle.tag}</span>
            </div>
          ...
          <div className="flex gap-3">
            <button onClick={onReject} className="flex-1 py-3 rounded-xl border border-slate-600 text-slate-400 hover:text-red-400 hover:border-red-700/50 transition-all text-sm font-medium">
              Decline
            </button>
            <button onClick={onAccept} className="flex-1 py-3 rounded-xl bg-[#0077B6] hover:bg-[#00B4D8] text-white transition-all text-sm font-semibold">
              Accept Ride
            </button>
          </div>
        </div>
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex items-center gap-3 bg-slate-800/60 border border-slate-700/50 rounded-xl p-3">
        <div className="w-10 h-10 rounded-full bg-[#0077B6]/20 border border-[#0077B6]/30 flex items-center justify-center text-lg text-[#00B4D8] font-bold shrink-0">U</div>
        <div className="flex-1 min-w-0">
          <p className="text-slate-100 text-sm font-medium">Passenger</p>
          <p className="text-slate-400 text-xs truncate">Booking #{String(booking?.bookingId ?? "").slice(-8)}</p>
        </div>
        <span className={`text-xs px-2 py-0.5 rounded-full border font-semibold shrink-0 ${typeStyle.color}`}>{typeStyle.tag}</span>
      </div>
      ...
      {action && (
        <button onClick={() => onStatusUpdate(action.next)} className={`w-full py-3 rounded-xl text-white font-semibold text-sm transition-all ${action.color}`}>
          {action.label}
        </button>
      )}
      {status === "Confirmed" && (
        <button onClick={onCancel} className="w-full py-2 rounded-xl border border-slate-700 text-slate-400 hover:text-red-400 hover:border-red-800/50 transition-all text-sm">
          Cancel booking
        </button>
      )}
    </div>
...
const DriverDashboard = () => {
  ...
  useEffect(() => {
    ...
    const offBookingTaken = on("booking_taken", ({ bookingId }) => {
      setIncomingRequest((prev) => prev?.bookingId === bookingId ? null : prev);
    });
    const offBookingCancelled = on("booking_cancelled", ({ bookingId }) => {
      setActiveBooking((prev) => {
        if (prev?.bookingId === bookingId) { setBookingStatus(null); return null; }
        return prev;
      });
      setIncomingRequest((prev) => prev?.bookingId === bookingId ? null : prev);
    });

    return () => { offNewBooking(); offBookingTaken(); offBookingCancelled(); };
  }, [socket, on]);
  ...
  const handleStatusUpdate = useCallback((nextStatus) => {
    ...
    emit("booking_status_update", { bookingId: activeBooking.bookingId, status: nextStatus });
    setBookingStatus(nextStatus);
    if (nextStatus === "Completed") {
      setStats((s) => ({ trips: s.trips + 1, earned: s.earned + (activeBooking.fare ?? 0) }));
      setTimeout(() => { setActiveBooking(null); setBookingStatus(null); }, 3000);
    }
  }, [activeBooking, emit]);
  ...
  const mapUserLocation = driverCoords ? [driverCoords.lat, driverCoords.lng] : null;

  return (
    <div className="relative flex flex-col h-screen overflow-hidden bg-slate-950">
      <div className="absolute inset-0 z-0">
        <Map userLocation={mapUserLocation} pickupLocation={mapPickup} dropLocation={mapDrop} />
      </div>

      <div className="relative z-10 w-full bg-slate-950/80 backdrop-blur-md flex items-center justify-between px-4 py-3 shadow-lg">
        <div className="flex items-center gap-3">
          <img src={ambulanceimage} className="w-8 h-8 rounded-full object-cover" alt="ResQRide" />
          <div className="hidden sm:block">
            <p className="text-slate-100 text-sm font-medium leading-tight">{user?.name ?? user?.userName ?? "Driver"}</p>
            <p className="text-slate-500 text-xs">{user?.vehicleNumber ?? ""}</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${connected ? "bg-emerald-900/50 text-emerald-400" : "bg-slate-800 text-slate-500"}`}>
            {connected ? "Connected" : "Reconnecting…"}
          </span>
          <OnlineToggle isOnline={isOnline} onToggle={handleToggleOnline} loading={onlineLoading} />
          <button onClick={handleLogout} className="text-slate-500 hover:text-slate-300 text-xs px-2 py-1 border border-slate-700 rounded-lg transition-colors">
            Logout
          </button>
        </div>
      </div>

      <div className="absolute bottom-0 left-0 right-0 z-10 max-w-lg mx-auto w-full bg-slate-950/85 backdrop-blur-md rounded-t-2xl shadow-2xl px-5 pb-6 pt-4">
        {!activeBooking && !incomingRequest && (
          <div className="space-y-4">
            <div className="grid grid-cols-3 gap-2">
              ...
              <StatCard label="Status"          value={isOnline ? "Online" : "Offline"} sub={isOnline ? "Waiting for ride" : "Go online to start"} />
            </div>
            {isOnline ? (
              <div className="flex flex-col items-center gap-2 py-3">
                ...
                {driverCoords && (
                  <p className="text-slate-600 text-xs">GPS active · {driverCoords.lat.toFixed(4)}, {driverCoords.lng.toFixed(4)}</p>
                )}
              </div>
            ) : (
              <div className="text-center py-4 space-y-2">
                ...
                <p className="text-slate-600 text-xs">Your location will be shared with nearby users</p>
              </div>
            )}
          </div>
        )}
          ...
          <div>
            {bookingStatus === "Completed" ? (
              <div className="flex flex-col items-center gap-3 py-4 text-center">
                <div className="w-14 h-14 rounded-full bg-emerald-900/40 border-2 border-emerald-500 flex items-center justify-center text-2xl">✓</div>
                <p className="text-slate-100 font-semibold">Ride Completed!</p>
                <p className="text-slate-400 text-sm">Fare collected: <span className="text-[#00B4D8] font-bold">₹{activeBooking.fare}</span></p>
      ...
      {incomingRequest && !activeBooking && (
        <BookingRequestModal request={incomingRequest} onAccept={handleAccept} onReject={handleReject} />
      )}
    </div>
  );
};
